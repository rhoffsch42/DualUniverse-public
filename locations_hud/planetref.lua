--[[ 
  ...
  Methods:
    ...
    PlanetarySystem:castBlocker - from a position in a given direction.
...
--
-- castBlocker - Find the closest body that intersects a "ray cast".
...
--
function PlanetarySystem:castBlocker(origin,
                                     direction,
                                     sizeCalculator,
                                     bodyIds)
    ...
    for i, id in ipairs(bodyIds) do
        ...
        if desc >= 0 then
            ...
            if dot + root > 0 then
                -- ignore single point intersections for closest body
                -- if the cast direction is outward from the center
                if i > 1                 or
                   dot - root > 0        or
                   dot/c_oV3:len() > -.2 then -- cos(A) < -.2  ->  A > 101deg
                   return body
               end
...
function BodyParameters:getDistance(worldCoordinates)
    return (worldCoordinates - self.center):len()
end
...
function BodyParameters:getGravity(worldCoordinates)
    local radial = self.center - worldCoordinates  -- directed towards body
